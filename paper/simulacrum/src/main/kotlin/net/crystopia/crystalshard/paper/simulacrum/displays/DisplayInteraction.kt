...
open class DisplayInteraction<T : Display>(open var entity: T) {
    ...
    fun onInteract(
        ...
        SimulacrumFactory.createEntityInstance<Interaction>(
        ...
        ClientPacketFactory.setEntityDataPacket(
            interaction.entityId, mutableListOf(
                EntityMetadata(
                    ...
                    index = 9, type = EntityDataSerializerType.FLOAT, value = size.second
                ), EntityMetadata(
                    index = 10, type = EntityDataSerializerType.BOOLEAN, value = true
                )
            )
        ) { packet ->
            packet.send(mutableListOf(player))
