package net.crystopia.crystalshard.paper.dhl.versions.v1_21_11.server
                ...
                override fun decode(
                    ...
                    Bukkit.getServer().scheduler.runTaskLater(
                        data.plugin, Runnable {
                            val buttonType = parseButton(msg.clickType, msg.buttonNum, msg.slotNum.toInt())
                            val clickType = msg.clickType.id()


                            var carried: ItemStack? = null
                            if (msg.carriedItem != null && msg.carriedItem is ActualItem) {
                                // TODO: HashedStack implementation...
                                carried = CraftItemStack.asBukkitCopy(
                                    net.minecraft.world.item.ItemStack(
                                        (msg.carriedItem as ActualItem).item,
                                        (msg.carriedItem as ActualItem).count
                                    )
                                )
                            }
                            val changed: MutableList<Slot> = mutableListOf()
                            msg.changedSlots.forEach { (i, stack) ->
                                if (stack is ActualItem) {
                                    // TODO: HashedStack implementation...
                                    changed.add(
                                        Slot(
                                            id = i,
                                            item = CraftItemStack.asBukkitCopy(
                                                net.minecraft.world.item.ItemStack(
                                                    stack.item,
                                                    stack.count
                                                )
